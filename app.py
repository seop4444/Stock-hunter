...
# 1. 페이지 설정
# ==========================================
st.set_page_config(page_title="Stock hunter v8 (눌림목 추가)", layout="wide")
st.title(" 🎯 Stock hunter: Pine Script Sync Hunter v8")
st.markdown("""
### ⚡ 멀티 타임프레임 고속 스캔 
**업데이트:** [🚀 급등대기] 패턴(장대양봉+대량거래 이후 거래량 급감 하락 패턴)이 스캐너에 추가되었습니다.
""")
...
# ==========================================
# 1.8. 포트폴리오 종목 분석 AI (실제 가격 & 실시간 뉴스 패치)
# ==========================================
...
if st.button("포트폴리오 일괄 분석하기", use_container_width=True):
    ...
    for idx, row in edited_df.iterrows():
        ...
        kr_code = "" # fdr 용 순수 6자리 코드
        
        # 1. 한글 이름 또는 코드 번호 처리 로직
        if not krx_df.empty:
        ...
        current_price = 0.0
        try:
            if kr_code: # 한국 주식은 fdr이 가장 빠르고 정확함
                df_price = fdr.DataReader(kr_code)
                if not df_price.empty:
                    current_price = df_price['Close'].iloc[-1]
            else: # 미국 주식은 yfinance
        ...
        if current_price == 0.0 and avg_price > 0:
            current_price = avg_price # 최후의 수단
...
# ==========================================
# 3.  분석 로직 (리샘플링 + 급등대기 로직 추가)
# ==========================================
def analyze_stock(ticker_info, timeframe):
    ticker = ticker_info['code']
    name = ticker_info['name']
    
    result = {"sniper": None, "entry": None, "touch": None, "sandwich": None, "vortex": None, "surge_prep": None}
    ...
    try:
        ...
        st_data = ta.supertrend(df['High'], df['Low'], df['Close'], length=10, multiplier=3.0)
        if st_data is not None:
            st_dir_col = next((c for c in st_data.columns if c.startswith('SUPERTd_')), None)
            if st_dir_col:
                df['stDir'] = st_data[st_dir_col]

        vortex = ta.vortex(df['High'], df['Low'], df['Close'], length=14)
        if vortex is not None:
            vip_col = next((c for c in vortex.columns if c.startswith('VTXP')), None)
            vim_col = next((c for c in vortex.columns if c.startswith('VTXM')), None)
            if vip_col and vim_col:
                df['VI_Plus'] = vortex[vip_col]
                df['VI_Minus'] = vortex[vim_col]
        ...
        try:
            ...
            is_long_d2 = rise_d2 >= 0.10
            
            body_d1 = abs(d1['Close'] - d1['Open']) / d1['Open']
            is_doji_d1 = body_d1 <= 0.03
            is_hold_d1 = d1['Close'] > d2['Open']
            
            rise_d0 = (d0['Close'] - d0['Open']) / d0['Open']
            is_long_d0 = rise_d0 >= 0.07
            
            if is_long_d2 and is_doji_d1 and is_hold_d1 and is_long_d0:
                result["sandwich"] = {
                    "종목명": name, "티커": ticker, "현재가": f"{round(d0['Close']):,}",
                    "패턴": "🥪 장대-도지-장대", "상승률": f"현재봉 +{round(rise_d0*100, 1)}%",
        ...
        try:
            if len(df) >= 3:
                d0 = df.iloc[-1]  # 오늘 (하락 + 거래량 감소)
                d1 = df.iloc[-2]  # 어제 (장대양봉 + 거래량 폭발)
                d2 = df.iloc[-3]  # 엊그제 (비교용)

                if d1['Open'] > 0 and d2['Volume'] > 0:
                    # 어제 조건: 6% 이상 몸통 장대양봉 & 전일비 거래량 2배 이상 폭발
                    is_long_d1 = ((d1['Close'] - d1['Open']) / d1['Open']) >= 0.06
                    vol_spike_d1 = d1['Volume'] >= (d2['Volume'] * 2.0)

                    # 오늘 조건: 전일 종가 대비 하락 & 거래량이 어제 대비 40% 이하로 급감
                    is_drop_d0 = d0['Close'] < d1['Close']
                    vol_drop_d0 = d0['Volume'] <= (d1['Volume'] * 0.40)

                    if is_long_d1 and vol_spike_d1 and is_drop_d0 and vol_drop_d0:
                        rise_pct = round(((d1['Close'] - d1['Open']) / d1['Open']) * 100, 1)
                        result["surge_prep"] = {
                            "종목명": name, "티커": ticker, "현재가": f"{round(d0['Close']):,}",
                            "패턴": "🚀 급등 대기 (눌림목)",
                            "조건": f"전일 {rise_pct}%↑ 대량거래 / 금일 거래급감",
                            "발견시간": time.strftime("%H:%M:%S")
                        }
        except:
            pass
        ...
        # --- 3-3. 보텍스 로직 ---
        try:
            if 'VI_Plus' in df.columns:
                curr = df.iloc[-1]
                prev = df.iloc[-2]
                
                prev_bear = prev['VI_Plus'] <= prev['VI_Minus']
                curr_bull = curr['VI_Plus'] > curr['VI_Minus']
                is_safe = curr['Close'] > curr['EMA20']
                
                if prev_bear and curr_bull and is_safe:
                    result["vortex"] = {
                        "종목명": name, "티커": ticker, "현재가": f"{round(curr['Close']):,}",
                        "지표값": f"VI+ {round(curr['VI_Plus'],2)}", "신호": "🌪️ 회오리 발생",
                        "발견시간": time.strftime("%H:%M:%S")
                    }
        except:
            pass

        # --- 3-4. 기타 스나이퍼/추세/터치 로직 ---
        for i in range(3):
            idx = -1 - i
            ...
            is_pump = (curr['Close'] - curr['Open']) > (curr['Open'] * 0.10)
            disparity = (curr['Close'] - curr['EMA20']) / curr['EMA20']
            is_overheat = (curr['RSI'] > 75) or (disparity > 0.20)
            is_danger = is_pump or is_overheat
            ...
            if timeframe == "4H":
                ...
                when_map = {0: "오늘", 1: "1일 전", 2: "2일 전"}
                when_txt = when_map.get(i, f"{i}일 전")

            touch_lines = []
            diff_20 = (curr['Close'] - curr['EMA20']) / curr['EMA20']
            if 0 <= diff_20 <= 0.02: touch_lines.append("20선")
                
            diff_60 = (curr['Close'] - curr['EMA60']) / curr['EMA60']
            if 0 <= diff_60 <= 0.02: touch_lines.append("60선")
                
            if not pd.isna(curr['EMA200']):
                diff_200 = (curr['Close'] - curr['EMA200']) / curr['EMA200']
                if 0 <= diff_200 <= 0.02: touch_lines.append("200선")

            if sig_sniper and result['sniper'] is None:
                result["sniper"] = {
                    "종목명": name, "티커": ticker, "현재가": f"{round(curr['Close']):,}",
                    "RSI": round(curr['RSI'], 1), "신호": f"🎯 스나이퍼 ({when_txt})",
                    "발견시간": time.strftime("%H:%M:%S")
                }
            if sig_normal and result['entry'] is None:
                result["entry"] = {
                    "종목명": name, "티커": ticker, "현재가": f"{round(curr['Close']):,}",
                    "RSI": round(curr['RSI'], 1), "신호": f"✅ 추세 진입 ({when_txt})",
                    "발견시간": time.strftime("%H:%M:%S")
                }
            if touch_lines and result['touch'] is None:
                lines_str = ", ".join(touch_lines)
                result["touch"] = {
                    "종목명": name, "티커": ticker, "현재가": f"{round(curr['Close']):,}",
                    "RSI": round(curr['RSI'], 1), "터치 라인": f"🧲 {lines_str} 지지 ({when_txt})",
                    "발견시간": time.strftime("%H:%M:%S")
                }
...
with st.sidebar:
    ...
    sound_on = st.checkbox("🔊 알람 켜기", value=True)
...
def process_stock_wrapper(stock):
    res_d = analyze_stock(stock, "1D")
    res_h = analyze_stock(stock, "4H")
    return stock, res_d, res_h
...
if st.session_state.running:
    ...
    while st.session_state.running:
        with placeholder.container():
            ...
            d_sniper, d_entry, d_touch, d_sandwich, d_vortex, d_surge = [], [], [], [], [], []
            h_sniper, h_entry, h_touch, h_sandwich, h_vortex, h_surge = [], [], [], [], [], []
            ...
            with main_tab1:
                t1_a, t1_b, t1_c, t1_d, t1_e, t1_f = st.tabs(["🎯 스나이퍼", "✅ 추세 진입", "🧲 지지선 터치", "🚀 급등 대기", "🥪 샌드위치", "🌪️ 보텍스"])
                d_table_sniper = t1_a.empty()
                d_table_entry = t1_b.empty()
                d_table_touch = t1_c.empty()
                d_table_surge = t1_f.empty()
                d_table_sandwich = t1_d.empty()
                d_table_vortex = t1_e.empty()
                
            with main_tab2:
                t2_a, t2_b, t2_c, t2_d, t2_e, t2_f = st.tabs(["🎯 스나이퍼", "✅ 추세 진입", "🧲 지지선 터치", "🚀 급등 대기", "🥪 샌드위치", "🌪️ 보텍스"])
                h_table_sniper = t2_a.empty()
                h_table_entry = t2_b.empty()
                h_table_touch = t2_c.empty()
                h_table_surge = t2_f.empty()
                h_table_sandwich = t2_d.empty()
                h_table_vortex = t2_e.empty()
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for i, future in enumerate(as_completed(futures)):
                    ...
                    if show_log:
                        status_text.text(f"Scanning... {stock_name} ({i+1}/{total})")
                    ...
                    try:
                        ...
                        if res_d['vortex']: d_vortex.append(res_d['vortex']); found=True
                        ...
                        if res_h['vortex']: h_vortex.append(res_h['vortex']); found=True
                        ...
                        if d_vortex: d_table_vortex.dataframe(pd.DataFrame(d_vortex), use_container_width=True)
                        ...
                        if h_vortex: h_table_vortex.dataframe(pd.DataFrame(h_vortex), use_container_width=True)
            ...
            has_result = (d_sniper or d_entry or d_touch or d_sandwich or d_vortex or d_surge or 
                          h_sniper or h_entry or h_touch or h_sandwich or h_vortex or h_surge)
            if not has_result:
